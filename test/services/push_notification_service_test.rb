...
class PushNotificationServiceTest < ActiveSupport::TestCase
  ...
  test 'removes expired subscriptions' do
    mock_response = OpenStruct.new(code: "410", body: "Gone")
  ...
  test 'logs and reports other push errors' do
    ...
    errors_sent = []
    original_send_error = Appsignal.method(:send_error) rescue nil
    ...
    if original_send_error
      Appsignal.define_singleton_method(:send_error, original_send_error)
    end
  end
